...
def sort_boundingbox(boxes = None, polys = None, text_label= None):
    ...
    if boxes is not None:
        ...
        numerical_order = list(range(len(texts_concat_bboxs[0])))
        for id,i in enumerate(numerical_order):
            for idi,j in enumerate(numerical_order):
                if texts_concat_bboxs[0][j][1]>texts_concat_bboxs[0][j+1][1]:
                    # if texts_concat_bboxs[0][j+1][7]-5>texts_concat_bboxs[0][j][1]:
                    #     line_index[j+1] = line_index[j]
                    # else:
                    #     line_index[j+1] = line_index[j]+1
                    temp= [texts_concat_bboxs[0][j+1].copy(),texts_concat_bboxs[1][j+1].clone()]
                    texts_concat_bboxs[0][j+1]=texts_concat_bboxs[0][j]
                    texts_concat_bboxs[1][j+1]=texts_concat_bboxs[1][j]
                    texts_concat_bboxs[0][j]= temp[0]
                    texts_concat_bboxs[1][j]= temp[1]
        for id,i in enumerate(range(len(texts_concat_bboxs[0])-2)):
            for idi,j in enumerate(range(len(texts_concat_bboxs[0])-2)):
        ...
        return texts_concat_bboxs[0],label_result,line_index
    else:
...
def main(config, resume: bool):
    ...
    for epoch in range(5):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(iter(data_input)):
            ...
            outputs['bboxes'],transcripss,line_index= sort_boundingbox(boxes = None, polys = outputs['bboxes'].cpu().numpy(), text_label=transcr )
            ...
            if (booxes is not None) and (len(booxes)>0):
                ...
                for box,label_box in zip(boxes,label_boxes):
                    ...
                    cv2.rectangle(img,[box[0,0],box[0,1]],[box[2,0],box[2,1]],(0, 255, 255), thickness= 3, lineType=cv2.LINE_8)        
            ...
            for id in range(outputs['transcripts'][0].shape[1]):
                # result_text = outputs['transcripts'][0][id] #nn.Softmax(dim=1)
