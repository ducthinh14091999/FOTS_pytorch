...
def show_box(image_name, image, box, transcirpt, isFeaturemap=False):
    ...
    cv2.imshow('img_tranfrom',ii.reshape(h, w, -1).astype(np.uint8))
...
if __name__ == '__main__':
    image = cv2.imread('F:/project_2/New_folder/data/downloads/7018607911915097410434632203221049212928000n-156817091076384.jpg')
    import pandas as pd
    gts = pd.read_csv('F:/project_2/New_folder/data/downloads/7018607911915097410434632203221049212928000n-156817091076384 - Copy.txt', header=None)
    for index, gt in gts.iterrows():
        x1, y1, x2, y2, x3, y3, x4, y4 = gt[1:]
        transcript = gt[0]
        show_box('F:/project_2/New_folder/data/downloads/7018607911915097410434632203221049212928000n-156817091076384.jpg',image, np.array([(x1, y1), (x2, y2), (x3, y3), (x4, y4)], dtype=np.int), transcript,True)
